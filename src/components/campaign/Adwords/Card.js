...
const navigateTo = (router, {company, workspace, folder, campaign}, fragment) => () => {
  router.push(`/company/${company}/workspace/${workspace}/folder/${folder}/campaign/${campaign}/edit/${fragment}`)
}

const AdwordsCampaign = ({params, campaign: {details, name}}, {messages, router}) =>
  <Fence canEditCampaign>{({canEditCampaign}) =>
    <Wrapper>
      <Info edit={canEditCampaign && navigateTo(router, params, 'name')}>
...
export default withState('editFragment', 'setEditFragment', false)(AdwordsCampaign)
